...
function App() {
  ...
  // const prime = useMemo(() => isPrime(num), [num, name]); 
  // 이런식으로 사용하게 된다면 num값이 변경될 경우 inPrime을 다시 계산하고 name의 값을 변경해도 inPrime을 다시 계산한다 즉 메모이제이션이 작동하지 않는다
  ...
  return (
    <>
      ...
      <div>
        ...
        <div>
          {name}(이)가 좋아하는 숫자 {num}: 소수가 {prime ? <span style={{ color: "blue" }}>맞습니다.</span> : <span style={{ color: "red" }}>아닙니다.</span>}
        </div>
      </div>
    </>
  );
