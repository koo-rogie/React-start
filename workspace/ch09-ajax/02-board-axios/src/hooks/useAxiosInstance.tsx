...
function useAxiosInstance() {
  ...
  instance.interceptors.request.use(
    (config) => {
      ...
      config.params = {
        delay: 2000,
        // 딜레이를 따로 주지 않아도 여기서 설정함,
        // 다만 코드상에 딜레이를 줄 경우 코드에서 지정한 딜레이가 적용됨
        ...config.params, // 기존 쿼리 스트링 복사
      };
      return config;
    },
    (error) => {
  ...
  instance.interceptors.response.use(
    (response) => {
      console.log("정상응답 인터셉터 호출", response);
    ...
    (error) => {
      ...
      return Promise.reject(new Error("잠시 후 다시 이용해주시길 바랍니다.")); 
      // 사용자에게는 에러코드를 적나하게 드러낼 이유가 없기때문에 이 작엄을 해줌
    }
  );
  return instance;
}
